...
export class AevaAvatar extends LitElement {
  static styles = [
    ...
    css`
      ...
      .status-offline {
        background-color: var(--aeva-text-muted-color, #94a3b8);
      }
      .status-away {
      ...
      .shape-square .status-badge,
      .shape-rounded .status-badge {
  ...
  render() {
    ...
    return html`
      <div class="${classMap(classes)}" aria-label="${this.name || 'User Avatar'}" role="img">
        <div class="avatar-inner">${content}</div>
        ${this.status !== 'none'
          ? html` <div class="status-badge status-${this.status}" title="${this.status}"></div> `
          : ''}
      </div>
    `;
  }
